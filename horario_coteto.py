# streamlit_horario_prioridad_completo_timewindow.py
...
pattern_sched = r'(Lunes|Martes|Mi[eé]rcoles|Jueves|Viernes|S[áa]bado|Domingo)\s+(\d{1,2}:\d{2})\s+a\s+(\d{1,2}:\d{2})'
...
# Main scheduling with time window
...
def compute_schedules(courses, ranking, min_days_free, banned, start_pref, end_pref, weights):
    ...
    for combo in all_combos:
        ...
        metrics.append((combo, avg_rank, win, free_days, veto_cnt, win_vio))
    ...
    scored = []
    for combo, avg, win, free, veto, win_vio in metrics:
        n1 = 1 - (avg / mx[0])
        n2 = 1 - (win / mx[1])
        n3 = free / mx[2]
        n4 = 1 - (veto / mx[3])
